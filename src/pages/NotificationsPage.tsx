...
import { Bell, Hash, Tag, ShoppingBag, UserPlus, Settings, Check, Trash2, Filter } from 'lucide-react';
...
export function NotificationsPage() {
  ...
  const groupNotificationsByTime = (notifs: any[]) => {
    return notifs.reduce((groups: any, notification) => {
      ...
      <div className="container max-w-4xl mx-auto px-4 py-6">
        <div className="space-y-6">
          ...
          <div className="flex flex-col space-y-4">
            ...
            {/* Mobile Action Buttons - 2 columns */}
          ...
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="grid w-full grid-cols-4">
              <TabsTrigger value="all" className="relative">
                ...
                {unreadCount > 0 && (
                  <Badge variant="destructive" className="ml-1 sm:ml-2 h-4 w-4 sm:h-5 sm:w-5 text-xs p-0 flex items-center justify-center">
                    {unreadCount}
                  </Badge>
                )}
              </TabsTrigger>
              <TabsTrigger value="mentions">
                <span className="hidden sm:inline">Nhắc đến</span>
            ...
            <TabsContent value={activeTab} className="space-y-6 mt-6">
              {Object.entries(groupedNotifications).map(([timeGroup, notifs]: [string, any]) => (
                <div key={timeGroup}>
                      ...
                      <div className="divide-y">
                        {notifs.map((notification: any, index: number) => {
                          ...
                          return (
                            <div
                              key={notification.id}
                              className={`p-4 transition-smooth hover:bg-muted/50 cursor-pointer ${
                            ...
                            >
                              <div className="flex items-start space-x-3">
                                <div className={`p-2 rounded-full flex-shrink-0 ${
                                  ...
                                  notification.type === 'hashtag' ? 'bg-green-100 text-green-600' :
                                  notification.type === 'saved_post' ? 'bg-orange-100 text-orange-600' :
                                      ...
                                      <div className="flex items-center justify-between flex-wrap gap-2">
                                        ...
                                        {notification.isDeleted && (
                                          <Button 
                                            ...
                                            className="text-xs"
                                          >
                                            <span className="hidden sm:inline">Xem sản phẩm tương tự</span>
                                            <span className="sm:hidden">Xem tương tự</span>
                                          </Button>
                                        )}
